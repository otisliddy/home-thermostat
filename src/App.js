...
import {hoursMinsToISOString, hoursMinsToSecondsFromNow, relativeDateAgo} from './util/time-helper';
...
const App = () => {
    ...
    async function handleOn(selection) {
        ...
        try {
            ...
            const status = statusHelper.createStatus(thingName, modes.ON.val, {
                duration: selection,
    ...
    async function handleTemperatureSchedule(thingName, dhwTargetTemperature) {
        try {
    ...
    return (
        <div id="root">
            ...
            <div id="homethermostat">
                <div className="tabs">
                    <button onClick={() => setThingName('ht-main')}
                            className={thingName === 'ht-main' ? 'active' : ''}>Oil
                    </button>
                    <button onClick={() => setThingName('ht-immersion')}
                            className={thingName === 'ht-immersion' ? 'active' : ''}>Immersion
                    </button>
                </div>
                <div disabled={scheduleModalShow}>
                    ...
                    <button onClick={() => handleTemperatureSchedule('ht-main', 42.0)}>DHW</button>
